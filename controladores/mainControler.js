...
exports.album = async (req, res) => {
    try {
        const fotosDir = path.join(__dirname, '..', 'publicos/img/fotos');
        let fotos = await fs.promises.readdir(fotosDir);
        fotos = fotos.sort((a, b) => b.localeCompare(a)); // Orden descendente
        console.log(fotos);
        res.render('layout', { 
            title   : 'Alumno', 
            content : 'album',
